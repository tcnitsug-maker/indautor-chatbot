...
// =======================================================================
// 15. MÉTRICAS (Dashboard) + Export XLSX
//    - GET  /admin/metrics/summary      (analyst+)
//    - GET  /admin/metrics/export-xlsx  (analyst+)
// =======================================================================
...
router.get("/metrics/summary", requireRole("analyst"), async (req, res) => {
  try {
    const now = new Date();
    ...
    const [
      ...
      CustomReply.countDocuments({}),
      CustomReply.countDocuments({ enabled: true }),
      Message.aggregate([
    ...
    const uniqueIPs = (uniqueIPsAgg && uniqueIPsAgg[0] && uniqueIPsAgg[0].n) ? uniqueIPsAgg[0].n : 0;
    ...
    res.json({
      ...
      uniqueIPs,
      blockedIPsActive: blockedActive,
      customRepliesTotal: customTotal,
      customRepliesEnabled: customEnabled,
      bySource: aiAgg,
      topIps,
      topTexts,
    ...
    console.error("Error metrics summary:", e);
    res.status(500).json({ error: "Error obteniendo métricas" });
  }
});

router.get("/metrics/export-xlsx", requireRole("analyst"), async (req, res) => {
  try {
    // Reusar summary para armar un XLSX presentable
    const now = new Date();
    const todayStart = _startOfDay(now);
    ...
    const summaryRows = [
      ...
      { Métrica: "Mensajes hoy", Valor: msgsToday },
      { Métrica: "IPs únicas", Valor: uniqueIPs },
      { Métrica: "IPs bloqueadas activas", Valor: blockedActive },
      { Métrica: "Respuestas personalizadas (total)", Valor: customTotal },
      { Métrica: "Respuestas personalizadas (habilitadas)", Valor: customEnabled },
      { Métrica: "Generado", Valor: now.toISOString() },
    ];

    const sourceRows = (bySource || []).map(s => ({ Fuente: s._id, Total: s.total }));
    const topIpsRows = (topIps || []).map(r => ({ IP: r._id, Total: r.total }));
    const topTextsRows = (topTexts || []).map(r => ({ Texto: r._id, Total: r.total }));

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(summaryRows), "Resumen");
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(sourceRows), "Por fuente");
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(topIpsRows), "Top IPs");
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(topTextsRows), "Top textos");

    const buf = XLSX.write(wb, { type: "buffer", bookType: "xlsx" });

    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", "attachment; filename=metricas_indarelin.xlsx");
    res.send(buf);
  } catch (e) {
    console.error("Error export metrics xlsx:", e);
